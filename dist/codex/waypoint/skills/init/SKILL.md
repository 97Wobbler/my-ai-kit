...
Users may call this skill as `$init` when unambiguous, or by saying `use waypoint:init`.
Use `request_user_input` only in Plan Mode. In Default mode, ask concise direct questions only when the target path, greenfield-vs-brownfield mode, or CLAUDE.md wrapper choice is blocking.
For manual corrections, use apply_patch. Prefer the bundled script for normal greenfield generation.
