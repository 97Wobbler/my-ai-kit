...
Users may call this skill as `$brief` or by saying `use lucid:brief`.

Ask clarification questions sparingly. In Plan Mode, structured user input may be available; in Default mode, ask concise direct questions only when a blocking fact prevents a useful brief.
