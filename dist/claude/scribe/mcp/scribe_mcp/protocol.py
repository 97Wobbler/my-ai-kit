"""Small JSON-RPC helpers for the Scribe MCP server.

This module intentionally uses only the Python standard library. It implements
the response and error handling needed by a stdio entrypoint without importing
...
def tool_error(message: str = "Tool error", data: Any | None = None) -> JsonRpcError:
    """Create a JSON-RPC server error for MCP tool failures."""

    return JsonRpcError(TOOL_ERROR, message, data)
