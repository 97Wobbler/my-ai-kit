...
Users may call this skill as `$branch light`, `$branch deep`, `$branch interview`, or by saying `use lucid:branch`.

In Plan Mode, structured user input may be available for blocking choices. In Default mode, ask concise direct questions in the conversation and wait for the user's answer.
