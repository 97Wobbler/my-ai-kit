...
Codex users may invoke this skill as `$transcribe`, `$scribe:transcribe`, or by asking Scribe to transcribe a local file path.
Use `request_user_input` only in Plan Mode, and only for truly blocking decisions, degraded fallback confirmation, or a short high-impact post-STT clarification packet.
In Default mode, ask concise plain-text questions and wait when a blocking answer is required.
