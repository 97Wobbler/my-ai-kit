...
Claude Code users may invoke this skill as `/scribe:canon`.
Use Claude Code's native user-question flow for the evidence-derived clarification packet, staged clarification turns, or truly blocking input/output-path decisions.
Do not run a separate upfront recording-context interview before the first evidence scan.
For long transcripts, use parallel agents only after defining non-overlapping segment boundaries and keep final canonical assembly in the main thread.
