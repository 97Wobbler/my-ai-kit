...
Users may call this skill as `$deep-audit` when unambiguous, or by saying `use waypoint:deep-audit`.
Use `request_user_input` only in Plan Mode. In Default mode, ask a concise direct question only when the target repository or the lens scope is blocking.
Spawn lens subagents with the runtime's native subagent mechanism when one is available. Prefer a read-only subagent profile (for example a custom agent configured with a read-only sandbox) when one exists; otherwise state the read-only constraint explicitly in every lens prompt. If this environment cannot spawn subagents, run the selected lenses sequentially in this session as isolated passes and label the report accordingly.
Use the `waypoint_audit` MCP tool for the inventory when it is available; otherwise use the bundled fallback script.
For apply mode, use `apply_patch` and preserve unrelated user changes.
