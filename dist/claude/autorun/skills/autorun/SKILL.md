...
5. **Track progress with the runtime's visible task tracker.** On RUN entry, create or update the visible task plan. Mark tasks in progress before delegation, and completed immediately after verification and commit.
...
Experimental worker execution tools, when exposed by the Autorun MCP server, are an opt-in adjunct to delegation. Use them only when the current runtime advertises the worker tools and the user has authorized Autorun RUN mode. Worker artifacts are stored outside the repository and do not replace `workplan.yaml`, visible task tracking, or main-session verification.
...
6. If MCP tools are available, create project-root `workplan.yaml` with `autorun_plan_create`, validate it with `autorun_plan_validate`, and use `autorun_plan_refine` to drive a validation/refinement loop. If `autorun_plan_refine` returns `needs_split`, split the indicated task, preferably through `autorun_task_split` when available, then re-run validation and refinement until the plan is valid or MCP fails.
...
4. **SCAN/PLAN**: With MCP available, call `autorun_next_batch` for runnable non-human-gated tasks. Otherwise, find automatic tasks where `done: false`, `status` is missing or `pending`, every `blocked_by` task is done, and `human_gate: null`, then batch independent tasks that do not touch the same files. Keep dependent chains foreground.
