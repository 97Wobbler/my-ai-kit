...
def format_channel_week_md(
    ...
    # week_key에서 주 시작일(월요일)과 종료일(일요일) 계산
    ...
    lines: list[str] = [
        "<!-- AUTO-GENERATED by slackbox. Do not edit. -->\n",
        f"# #{channel_name} — {week_key} ({week_start.strftime('%m/%d')}~{week_end_dt.strftime('%m/%d')}) — {len(messages)}건\n",
    ]
...
def _format_weekly_md(
    ...
    for week_key in sorted(by_week.keys()):
        ...
        lines.append(
            f"# {week_key} ({week_start.strftime('%m/%d')}~{week_end_dt.strftime('%m/%d')}) — {len(msgs)}건\n"
        )
        lines.append(
            f"채널: {', '.join(f'#{c}({n})' for c, n in sorted(ch_counts.items(), key=lambda x: -x[1]))}\n"
        )
