...
Users may call this skill as `$audit` when unambiguous, or by saying `use waypoint:audit`.
Use `request_user_input` only in Plan Mode. In Default mode, ask a concise direct question only when the target repository or requested apply scope is blocking.
