...
| 모드 | Agent 파라미터 | 조건 | VERIFY 시점 |
|---|---|---|---|
| foreground | `run_in_background: false` (기본값) | 후속이 결과에 즉시 의존, 또는 검수 후 체인 이어가기 | EXEC 직후 즉시 |
| background | `run_in_background: true` | 독립 태스크, 후속 없음, 시간 오래 걸림 | 다음 루프의 DRAIN에서 |
...
```
Agent({
  description: "Verify T05 implementation",
  subagent_type: "Explore",
  prompt: "T05가 변경한 파일 X, Y, Z를 읽고 다음을 확인해주세요:
...
5. TaskCreate로 미완료 태스크 재등록 (사전 로딩 5번)
